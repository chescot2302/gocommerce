...
import com.gocommerce.server.model.logic.LogicTienda;
...
public class MantenimientoTienda {
    public static List<Tienda> listarTiendaxSucursal(String clavePublica,String idSucursal) throws UnknownException{
        ...
        try{
            ...
            if(!pm.isClosed()){
                LogicTienda logic = new LogicTienda(pm);  
                JDOConnection cnxJDO = pm.getDataStoreConnection();
                cnx =(Connection)cnxJDO.getNativeConnection();                
                List<Tienda> lista=logic.getListarBeanxSucursal(cnx,idSucursal);  
                cnx.close();
                return lista;
            }else{
