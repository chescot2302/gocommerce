...
import com.gocommerce.server.model.logic.LogicTipoVenta;
 ...
 * @author SISTEMAS
 */
public class MantenimientoTipoVenta {
    public static List<TipoVenta> listar(String clavePublica) throws UnknownException{
        ...
        try{
            ...
            if(!pm.isClosed()){
                LogicTipoVenta logic = new LogicTipoVenta(pm);  
                JDOConnection cnxJDO = pm.getDataStoreConnection();
                cnx =(Connection)cnxJDO.getNativeConnection();                
                List<TipoVenta> lista=(List)logic.getListarBean(cnx);  
                cnx.close();
                return lista;
            }else{
            ...
            if (cnx != null) {
                try {
                    ...
                    Logger.getLogger(MantenimientoTipoVenta.class.getName()).log(Level.SEVERE, null, ex);
                    throw new UnknownException(ex.getMessage());
                }
            }
            if (!pm.isClosed()) {                
