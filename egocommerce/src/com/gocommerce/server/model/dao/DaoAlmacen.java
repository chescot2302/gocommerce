...
public class DaoAlmacen {
    ...
    public DaoAlmacen(PersistenceManager pm) {
        this.pm = pm;
    }
    ...
    public ArrayList getListarBean(ArrayList<Parametro> param, Connection objCnx) throws UnknownException {
        try {
            ArrayList result;
            String fun = "{call NewSpAlmacenStockItem(?,?)}";
    ...
    public ArrayList listarAlmXLocalidad(ArrayList<Parametro> param, Connection objCnx) throws UnknownException {
        try {
            ArrayList result;
            String fun = "{call NewSpAlmLocalidaStockItem(?,?,?)}";
            Consultas query = new Consultas();
            result = query.funcionSQLServerResultSet(fun, param, objCnx);
            result.add(objCnx);
            return result;
