 ...
 * @author SISTEMAS
 */
public class GridRegistroVenta extends DataGrid<DetalleVentaProxy> {
    private UIrvEntradaImpl uiPadre;
    ...
    private void initColumns() {
        ...
        this.setColumnWidth(asignar, 6, Unit.EM);
        this.setColumnWidth(ver, 5, Unit.EM);
    ...
    private void initEditColumns() {
        ...
        asignar.setFieldUpdater(new FieldUpdater<DetalleVentaProxy, String>() {

            @Override
            public void update(int index, DetalleVentaProxy object, String value) {                    
                        UIItemPlan ui = new UIItemPlan(uiPadre,object);
                        ui.setModo("ADD","E");                        
                        ui.loadData();                        
                        ui.show();
            }
        ...
        ver.setFieldUpdater(new FieldUpdater<DetalleVentaProxy, String>() {
            ...
            public void update(int index, DetalleVentaProxy object, String value) {                    
                        UIItemPlan ui = new UIItemPlan(uiPadre,object);
                        ui.setModo("REMOVE","L");
                        ui.getGrid().setData(object.getEquipos());                        
                        ui.show();
            }
        });
    ...
    private void calcularMontos(int index, DetalleVentaProxy object, String value) {
        if (!FieldVerifier.notIsNumberPositivo(value, "Precio")) {
            ...
            beanEdit.setPrecioPlan(valPrecioVenta);
    ...
    private Column<DetalleVentaProxy, String> asignar
    ...
    public int existe(String idItem,Integer idLista) {
        int val = -1;
        for (int i = 0; i < data.size(); i++) {
            if (data.get(i).getIdItem().equalsIgnoreCase(idItem) && data.get(i).getIdLista().equals(idLista)) {
                return i;
            }
        }
        return val;
    ...
    public void setUiPadre(UIrvEntradaImpl uiPadre) {
